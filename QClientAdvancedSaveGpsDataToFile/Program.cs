...
const string outputFileName = "#output_log.txt";
const string gpggaFileName = "#gpgga_log.txt";

Console.WriteLine("QClient Advanced - Save GPS Data to File");
Console.WriteLine("This example will demonstrate how to stream and unpack GPS Channel Data to a file.");
Console.WriteLine("Module and Channel setup should be made via QAcquire. Ensure a GPS channel is enabled.");
Console.WriteLine(string.Empty);

// Establish a connection to QServer.
...
// Open a socket to stream the data to your PC.
    ...
    while (bytesLeft > 0)
    {
        ...
        switch (genericChannelHeader.ChannelType)
        {
            ...
            case ChannelTypes.Gps:
                ...
                message.AppendLine($"UBlox Timestamp: {gpsChannelHeader.Timestamp}, Accuracy: {gpsChannelHeader.AccuracyInNanoSeconds} ns, Is Leap Seconds Valid: {gpsChannelHeader.IsLeapSecondsValid == 1}, Leap Seconds: {gpsChannelHeader.LeapSeconds}");
                ...
                if (startIndex > 0)
                {
                    ...
                    File.AppendAllText(gpggaFileName, Invariant($"{gpsChannelHeader.Timestamp:R},{ggaMessage},{Environment.NewLine}"));
                }
